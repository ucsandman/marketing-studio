...
if (pending.length) {
  ...
  const out = run(`node feeders/audio/client.mjs vo --project "${workspace.projectRoot}" --script "${scriptPath}" --out "${publicDir}" --brand postflop --timestamps`);
...
const outPath = join(workspace.propsDir, 'postflop-film-audio.json');
