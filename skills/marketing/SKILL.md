...
| 5 | /social-clip × N | Reuse demo/logo footage per platform. SocialClip renders silent; when the intake asked for audio, run `node scripts/score-social-clip.mjs <brand> <clipId> --vo <act>` per clip (music bed plus the ONE VO line that matches the plate, e.g. hook for the X clip), master it with master-audio, and leave `<clipId>-final.mp4` in out/<brand>/ so build-postkit picks it over the generic matrix row (postflop shipped a silent X clip carrying the LinkedIn headline, 2026-09-01) |
