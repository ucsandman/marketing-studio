...
| Content brief gatherer | `scripts/derive-brief.mjs <brand> <productRepo> [--url]` | grounding -> out/<brand>/marketing/brief-inputs.json; agent synthesizes brief.json (zod: `studio/src/lib/brief.ts` — includes grounding sections: audience, customerLanguage, objections, JTBD switchingForces, sourced proofPoints; hook/CTA formulas in `skills/marketing/references/hook-formulas.md`); build-launch-props overlays brief copy when brief.json exists |
